# -*- coding: utf-8 -*-
"""visualizations_rosicrucian.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1kLmgReYzmGXrzLJ0iSsdptTiyxHcfTo6
"""
...
rosicrucian_cipher_numpy = {key:np.array([0]+[int(x) for x in rosicrucian_cipher_dict[key]] + [0, 0])
...
with sn.plotting_context("notebook", font_scale=4):
    ...
    for x, k in enumerate(rosicrucian_cipher_numpy.keys()):
        ax = fig.add_subplot(spec[x%15, int(x/15)])
        ax.text(0.5, 0.5, str(k), fontsize=15, ha='center', va='center', color='black')

        ax.imshow(rosicrucian_cipher_numpy[k][np.newaxis], aspect='auto')
        ax.set_xticks([])
        ax.set_yticks([])

TCN_perfs = pd.read_pickle(join("perf", "tcn_rosicrucian_test.dill"))
SITH_perfs = pd.read_pickle(join("perf", "sithcon_rosicrucian_test.dill"))
print(SITH_perfs.head())
print(SITH_perfs.perf)
print(TCN_perfs.perf)
print(SITH_perfs)
print(TCN_perfs)
...
plt.savefig(join('figs', 'Rosicrucian_decoder_Full.png'), dpi=100)
plt.savefig(join('figs', 'Rosicrucian_decoder_Full.pdf'), dpi=100)
